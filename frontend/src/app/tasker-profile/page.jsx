...
import { useRouter } from "next/navigation"; // <-- Change from 'react-router-dom'
import Header from "../../components/Header"; // Make sure the path is correct
import Footer from "../../components/Footer"; // Make sure the path is correct
import { useUser } from '../../context/UserContext'; // You will likely need this too
...
const Profile = () => {
  ...
  // Fetch fresh profile data on mount to ensure we have firstName/lastName
  useEffect(() => {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/users/profile', {
        ...
        body: JSON.stringify({ name: userName, firstName, lastName, email, password, city, address, skills, profileImage, aboutMe, hourlyRate, skillRates, skillImages, gallery }),
      });
      ...
      if (res.ok) {
        ...
        setTimeout(() => {
          if (data.role === 'tasker') {
            router.push(`/handyman/${data._id}`);
          } else if (data.city) {
            router.push(`/city/${encodeURIComponent(data.city)}`);
          } else {
    ...
    <div className="min-h-screen bg-zinc-950 font-sans text-zinc-100 flex flex-col justify-between">
      ...
      <div className="flex flex-col items-center pt-10 pb-20">
        ...
        <div className="flex gap-4 mb-4">
          <div className="w-1/2">
            <label className="block text-zinc-400 mb-2 text-sm">First Name <span className="text-red-500">*</span></label>
            <input
              type="text"
              className="w-full p-3 bg-zinc-950 border border-zinc-800 rounded text-white focus:outline-none focus:border-[#D4AF37]"
              value={firstName}
          ...
          <div className="w-1/2">
            <label className="block text-zinc-400 mb-2 text-sm">Last Name <span className="text-red-500">*</span></label>
            <input
              type="text"
              className="w-full p-3 bg-zinc-950 border border-zinc-800 rounded text-white focus:outline-none focus:border-[#D4AF37]"
              value={lastName}
