  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.role === 'tasker' && !data.isVerified) {
    ...
    <div className="min-h-screen bg-zinc-950 font-sans text-zinc-100 flex flex-col">
      ...
      <main className="flex-grow flex justify-center items-center py-12 px-4">
        <form onSubmit={submitHandler} className="bg-zinc-900 p-8 rounded-2xl shadow-2xl w-96 border border-zinc-800">
          ...
          <p className="text-red-500 text-xs text-center mb-4 font-bold">All fields are required to enter or it won't register at all.</p>
          ...
          <p className="mt-6 text-center text-sm text-zinc-500">
            Have an account? <Link href="/login" className="text-[#D4AF37] hover:underline">Login</Link>
          </p>
        </form>
      </main>
      <Footer />
